...
class UIUtils {
	...
	public static AddIconToButton(btn:Laya.Sprite,iconUrl:string,rightOffsetX?:number){
		let icon=GameUtils.CreateSprite(iconUrl,1);
		btn.addChild(icon);
	...
	//设置是否变灰，可能有性能问题
	public static SetGray(spr:Laya.Sprite,b:boolean){
		if(b){
	...
	//
	public static CreateBlockBG(url: string): Laya.Sprite {
		let blockBG = GameUtils.CreateSprite(url, 1);
		blockBG.pivotX=0;
		blockBG.pivotY=0;
	...
	//设置spr到指定尺寸
	public static SetSize(spr: Laya.Sprite, width: number, height: number) {
		let scalex = width / spr.width;
		let scaley = height / spr.height;
	...
	public static CreateSimpleTextField(fontName: string, x: number, y: number, w: number, h: number, parent: Laya.Sprite): Laya.Text {
		let t = new Laya.Text();
		t.x = x;
		t.y = y;
	...
	//创建简单的BTN
	public static CreateSimpleBtn(url: string, caller: any, fun: Function): Laya.Sprite {
